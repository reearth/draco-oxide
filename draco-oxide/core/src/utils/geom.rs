...
pub fn point_to_face_distance_3d<F: Float>(p: NdVector<3, F>, face: [NdVector<3, F>; 3]) -> F {
    ...
    let p_onto_plane_inside_face = (p_onto_plane - face[0]).dot(face[1] - face[0])
        * (face[2] - face[0]).dot(face[1] - face[0])
        > F::zero()
        && (p_onto_plane - face[1]).dot(face[2] - face[1])
            * (face[0] - face[1]).dot(face[2] - face[1])
            > F::zero()
        && (p_onto_plane - face[2]).dot(face[0] - face[2])
            * (face[1] - face[2]).dot(face[0] - face[2])
            > F::zero();
...
pub fn point_to_line_distance_3d<F: Float>(p: NdVector<3, F>, line: [NdVector<3, F>; 2]) -> F {
    ...
    // Distance is |perp|. Computing it via |perp.dot(unit_perp)| trips
    // a safety_assert when p is on the line (perp ≈ 0, normalize → NaN).
    // Take the norm directly — equivalent and degenerate-safe.
    perp.norm()
}
