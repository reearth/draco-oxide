...
pub struct AttributeCornerTable {
    ...
    pub vertex_to_attribute_map: VecVertexIdx<AttributeValueIdx>,
    /// A vector that indicates whether the edge opposite to the corner is a seam edge.
    pub is_edge_on_seam: Vec<bool>,
    pub is_vertex_on_seam: Vec<bool>,
    pub left_most_corners: Vec<CornerIdx>,
    num_vertices: usize,
}
...
impl AttributeCornerTable {
    ...
    #[allow(unused)]
    pub fn get_vertex_to_attribute_map(&self) -> &VecVertexIdx<AttributeValueIdx> {
        &self.vertex_to_attribute_map
    }

    pub fn vertex_idx(&self, c: CornerIdx) -> VertexIdx {
    ...
    pub fn next(&self, c: CornerIdx, corner_table: &CornerTable) -> CornerIdx {
        corner_table.next(c)
    }

    pub fn previous(&self, c: CornerIdx, corner_table: &CornerTable) -> CornerIdx {
        corner_table.previous(c)
    }

    pub fn opposite(&self, c: CornerIdx, corner_table: &CornerTable) -> Option<CornerIdx> {
