...
#[derive(Debug, Clone)]
pub struct AllInclusiveCornerTable<'faces> {
...
#[derive(Debug, Clone)]
pub struct RefAttributeCornerTable<'faces, 'table> {
    idx: usize,
...
impl<'faces, 'table> GenericCornerTable for RefAttributeCornerTable<'faces, 'table> {
    ...
    fn point_idx(&self, corner: CornerIdx) -> crate::types::PointIdx {
        self.corner_table.universal_corner_table().point_idx(corner)
    }
    fn vertex_idx(&self, corner: CornerIdx) -> crate::types::VertexIdx {
        self.corner_table
            .attribute_tables
            .get(self.idx)
            .unwrap()
    ...
    fn next(&self, c: CornerIdx) -> CornerIdx {
        self.corner_table
            ...
            .get(self.idx)
            .unwrap()
            .next(c, &self.corner_table.universal)
    }
    fn previous(&self, c: CornerIdx) -> CornerIdx {
        self.corner_table
            ...
            .get(self.idx)
            .unwrap()
            .previous(c, &self.corner_table.universal)
    }
    fn opposite(&self, c: CornerIdx) -> Option<CornerIdx> {
        self.corner_table
            ...
            .get(self.idx)
            .unwrap()
            .opposite(c, &self.corner_table.universal)
    }
    fn left_most_corner(&self, vertex: crate::types::VertexIdx) -> CornerIdx {
        self.corner_table
            .attribute_tables
            .get(self.idx)
            .unwrap()
            .left_most_corner(vertex)
    }
    fn vertex_to_attribute_map(&self) -> Option<&VecVertexIdx<AttributeValueIdx>> {
        self.corner_table
            .attribute_tables
            .get(self.idx)
            .map(|att| att.get_vertex_to_attribute_map())
    }
}
