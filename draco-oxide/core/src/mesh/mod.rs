...
use crate::types::{NdVector, PointIdx};
...
impl Mesh {
    ...
    pub fn diff_l2_norm(&self, other: &Self) -> f64 {
        ...
        for ((_, pos_att), (_, other_pos_att)) in pos_att_iter.zip(other_pos_att_iter) {
            ...
            sum_of_squared_dist +=
                sum_of_squared_dist_unpack_datatype(pos_att, faces, other_pos_att, other_faces);
        }
    ...
    NdVector<3, F>: Vector<3, Component = F>,
{
    ...
    if other_pos_att.get_num_components() != 3 {
        panic!(
            "Position attribute must have 3 components, but the second mesh has {} components",
            other_pos_att.get_num_components()
        );
    }
