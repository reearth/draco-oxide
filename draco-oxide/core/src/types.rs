...
pub trait Vector<const N: usize>:
    ...
    + ops::Add<Output = Self>
    + ops::Sub<Output = Self>
    + ops::Mul<Self::Component, Output = Self>
    + ops::Div<Self::Component, Output = Self>
    + ops::AddAssign
    + ops::SubAssign
    + ops::Mul<Self::Component, Output = Self>
    + ops::Div<Self::Component, Output = Self>
    ...
    fn cross(self, other: Self) -> Self {
        if N == 3 {
            unsafe {
                ...
                *out.get_unchecked_mut(0) = *self.data.get_unchecked(1)
                    * *other.data.get_unchecked(2)
                    - *self.data.get_unchecked(2) * *other.data.get_unchecked(1);
                *out.get_unchecked_mut(1) = *self.data.get_unchecked(2)
                    * *other.data.get_unchecked(0)
                    - *self.data.get_unchecked(0) * *other.data.get_unchecked(2);
                *out.get_unchecked_mut(2) = *self.data.get_unchecked(0)
                    * *other.data.get_unchecked(1)
                    - *self.data.get_unchecked(1) * *other.data.get_unchecked(0);
                NdVector { data: out }
            }
        } else {
...
mod tests {
    ...
    fn test_ndvector_add() {
        let vector1 = NdVector {
            data: [1.0, 2.0, 3.0],
        };
        let vector2 = NdVector {
            data: [4.0, 5.0, 6.0],
        };
    ...
    fn test_ndvector_elem_div() {
        let vector1 = NdVector {
            data: [4.0, 10.0, 18.0, 2.0],
        };
        let vector2 = NdVector {
            data: [2.0, 5.0, 3.0, 4.0],
        };
