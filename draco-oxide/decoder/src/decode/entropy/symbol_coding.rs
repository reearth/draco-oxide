    ...
    R: ByteReader,
{
    ...
    match method {
        ...
        SymbolEncodingMethod::DirectCoded => decode_symbols_direcd_coded(num_symbols, reader),
    }
}
...
#[allow(unused)] // TODO: Remove this when decoder is fully implemented
pub fn decode_symbols_direcd_coded<R>(num_symbols: usize, reader: &mut R) -> Result<Vec<u64>, Err>
where
    R: ByteReader,
{
...
pub fn decode_symbols_direcd_coded_precision_unwrapped<
    R,
...
where
    R: ByteReader,
{
    let mut decoder: RansSymbolDecoder<R, NUM_SYMBOLS_BIT_LENGTH, RANS_PRECISION> =
        RansSymbolDecoder::new(reader)?;
