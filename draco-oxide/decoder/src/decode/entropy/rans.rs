use draco_oxide_core::bit_coder::{ByteReader, ReaderErr, ReverseByteReader};
use draco_oxide_core::codec::entropy::{
...
where
    R: ReverseByteReader,
{
    pub(crate) fn new<ForwardReader>(
    ...
    where
        ForwardReader: ByteReader<Rev = R>,
    {
...
pub(crate) struct RansSymbolDecoder<
    R,
    ...
    R: ByteReader,
{
        ...
        R: ByteReader,
    {
        ...
        while i < num_symbols {
            ...
            if token == 3 {
                ...
                if i + offset >= num_symbols {
                    Err(Err::RansSymbolDecoderError(
                        "Invalid offset for frequency counts",
                    ))?;
                }
                for j in 0..=offset {
                    freq_counts[i + j] = 0;
        ...
        let rans_decoder: RansDecoder<_, RANS_PRECISION> =
            RansDecoder::new(reader, offset, freq_counts.clone(), None)?;
...
mod tests {
    ...
    fn test_rans_decoder() {
        ...
        let mut decoder: RansDecoder<_> =
            RansDecoder::new(&mut reader, len, freq_counts, None).unwrap();
    ...
    fn test_rabs_coder() {
        ...
        let len = buffer.len();
        let mut reader = buffer.into_iter();
        let mut decoder: RabsDecoder<_> =
            RabsDecoder::new(&mut reader, len, num_zeros, None).unwrap();
